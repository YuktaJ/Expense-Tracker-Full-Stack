...
async function refresh() {
    ...
    try {
        let result = await axios.get(`http://52.53.205.42:3000/expenses?page=${page}&item=${localStorage.getItem("item_perPage")}`, { headers: { "Authorization": token } });
...
async function getExpenses(page) {
    try {
        console.log(page)
        let token = localStorage.getItem("token");
        let expenses = await axios.get(`http://52.53.205.42:3000/expenses?page=${page}&item=${localStorage.getItem("item_perPage")}`, { headers: { "Authorization": token } });
...
async function storeExpenses(obj) {
    try {
        let token = localStorage.getItem("token")
        let expense = await axios.post("http://52.53.205.42:3000/expenses", obj, { headers: { "Authorization": token } })
...
function showExpenseOnScreen(obj) {
    ...
    deleteBtn.onclick = async () => {
        ...
        try {
            let token = localStorage.getItem('token');
            await axios.delete("http://52.53.205.42:3000/deleteExpense/" + obj.id, { headers: { "Authorization": token } });
        } catch (error) {
            console.log("Error")
        }
    }
...
async function premiumUser(event) {
    try {
        let token = localStorage.getItem("token");
        let res = await axios.get("http://52.53.205.42:3000/premiumMembership",
        ...
        var options = {
            ...
            handler: async function (res) {
                const response = await axios.post("http://52.53.205.42:3000/updateTransaction",
                    {
...
async function leaderBoard(event) {
    ...
    try {
        ...
        let res = await axios.get("http://52.53.205.42:3000/showLeaderBoard");
...
downloadBtn.onclick = async () => {
    try {
        let token = localStorage.getItem("token");
        let result = await axios.get("http://52.53.205.42:3000/downloadedExp", { headers: { "Authorization": token } });
...
async function DownloadedFiles() {
    ...
    try {
        let token = localStorage.getItem("token");
        let res = await axios.get("http://52.53.205.42:3000/downloadhistory", { headers: { "Authorization": token } });
